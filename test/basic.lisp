...
(deftest split ()
  (let ((rope (rope:make-rope *string-2*)))
    (multiple-value-bind (ante post) (rope::split-rope rope 20)
      (is (string= (subseq *string-2* 0 20)
                   (rope:write-rope ante nil)))
      (is (string= (subseq *string-2* 20)
                   (rope:write-rope post nil))))))
